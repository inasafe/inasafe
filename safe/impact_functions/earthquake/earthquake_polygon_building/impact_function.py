# coding=utf-8
"""InaSAFE Disaster risk tool by Australian Aid - Earthquake Polygon on Building
Impact Function.
...
from safe.impact_functions.earthquake.earthquake_polygon_building\
    .metadata_definitions import EarthquakePolygonBuildingFunctionMetadata
...
class EarthquakePolygonBuildingFunction(
        ImpactFunction,
        BuildingExposureReportMixin):
    """Impact Function for Earthquake Polygon on Building."""

    _metadata = EarthquakePolygonBuildingFunctionMetadata()

    def __init__(self):
        super(EarthquakePolygonBuildingFunction, self).__init__()
    ...
    def run(self, layers=None):
        """Risk plugin for earthquake polygon hazard on building/structure.

        Counts number of building exposed to each earthquake hazard zones.

        :param layers: List of layers expected to contain.
                * hazard_layer: Hazard layer of earthquake
                * exposure_layer: Vector layer of structure data on
                the same grid as hazard_layer

        :returns: Map of building exposed to earthquake hazard zones.
        ...
        map_title = tr('Buildings affected by earthquake hazard zone')
        ...
        impact_layer = Vector(
            ...
            name=tr('Buildings affected by earthquake hazard zone'),
