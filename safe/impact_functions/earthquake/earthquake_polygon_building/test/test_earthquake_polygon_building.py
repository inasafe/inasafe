...
from safe.impact_functions.earthquake.earthquake_polygon_building.impact_function \
    import EarthquakePolygonBuildingFunction
from safe.test.utilities import test_data_path
from safe.storage.core import read_layer


class TestEarthquakePolygonBuildingFunction(unittest.TestCase):
    """Test for Earthquake Polygon on Building Impact Function."""

    def setUp(self):
        registry = ImpactFunctionManager().registry
        registry.clear()
        registry.register(EarthquakePolygonBuildingFunction)

    def test_run(self):
        """TestVolcanoPolygonBuildingFunction: Test running the IF."""
        earthquake_path = test_data_path('hazard', 'volcano_krb.shp')
        building_path = test_data_path('exposure', 'buildings.shp')

        hazard_layer = read_layer(earthquake_path)
        exposure_layer = read_layer(building_path)

        impact_function = EarthquakePolygonBuildingFunction.instance()
    ...
    def test_filter(self):
        ...
        expected = ImpactFunctionManager().get_function_id(
            EarthquakePolygonBuildingFunction)
