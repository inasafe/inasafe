...
style_classes = [
    ...
    dict(label=tr('Low'), colour='#38A800', quantity=5, transparency=0),
...
style_classes = [
    dict(colour='#EEFFEE', quantity=0.01, transparency=100, label=tr('Low')),
    ...
    dict(colour='#660000', quantity=8.30, transparency=0,
         label=tr('High'))]
earthquake_fatality_style = dict(
    target_field=None, style_classes=style_classes)
...
def generate_categorical_color_ramp(
        class_count,
        reverse_hue=True,
        saturation=0.5,
        value=0.7):
